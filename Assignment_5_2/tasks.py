...
def _setup_addon_common(c,
    ...
    if capstone or all:
        addon_list.append(addon_dict["capstone"])
    ...
    # )
    # addons_capstone = op.abspath(
    #     op.join(CONDA_ENV_FOLDER, f"addon-capstone-{env}.yml")
    # )
    ...
    #     if capstone:
    #         c.run(
    #             f"conda env update --name {ENV_PREFIX}-{DEV_ENV} --file {addons_capstone}"
    #         )
    #     if pyspark:
    #         c.run(
...
def calculate_complexity_score(path):
    """Calculates the Halstead complexity metrics using Radon
    """
    os.system("radon hal .")
